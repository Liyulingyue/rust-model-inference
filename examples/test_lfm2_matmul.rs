...
use rust_model_inference::ops::quantize_q8_0_into;
    ...
    matmul_q8_0_quantized_scalar_range(&weight, &input_q8, &input_scales, &mut out_q8, n_in, 0, n_out);
...
fn main() {
    ...
    // Load BOS embedding
    let emb_tensor = loader.tensors().iter().find(|t| t.name == "token_embd.weight").unwrap();
    ...
    let normed: Vec<f32> = emb.iter().zip(attn_norm.iter()).map(|(&x, &w)| x * scale * w).collect();
    println!("normed first 8: {:?}", &normed[..8]);
    
    // Load in_proj weight (Q8_0)
    let weight = loader.tensor_slice("blk.0.shortconv.in_proj.weight").unwrap();
    let n_in = 2048usize;
    let n_out = 6144usize;
    ...
    for b in 0..blocks_per_row {
        ...
        let scale = half::f16::from_bits(u16::from_le_bytes([weight[off], weight[off+1]])).to_f32();
        for j in 0..32 {
    ...
    println!("\nMy Rust (F32 matmul):");
    println!("  bx[0,0] = {}", bx[0]);
    println!("  bx[0,1] = {} (vs llama -1.3976)", bx[1]);
    println!("  bx[0,2] = {} (vs llama -0.0058)", bx[2]);
    println!("  bx[0,2045] = {} (vs llama -0.0058)", bx[2045]);
    println!("  bx[0,2046] = {} (vs llama -0.00044)", bx[2046]);
    println!("  bx[0,2047] = {} (vs llama -0.0058)", bx[2047]);
    
    println!("\nMy Q8 matmul:");
    println!("  bx[0,0] = {}", out_q8[0] * out_q8[4096 + 0]);
    println!("  bx[0,1] = {} (vs llama -1.3976)", out_q8[1] * out_q8[4097]);
    println!("  bx[0,2] = {} (vs llama -0.0058)", out_q8[2] * out_q8[4098]);
    ...
    println!("\nMy bcx[0, 0..3] = [{}, {}, {}]", out_q8[0], out_q8[1], out_q8[2]);
    ...
    println!("\nDebug: weight[4098] scales (first 5): {:?}", &debug_scales[..5]);
    ...
    println!("out_f32[max_idx]={}, out_q8[max_idx]={}", out_f32[max_idx], out_q8[max_idx]);
    
    // Print first 5 values
    println!("\nFirst 8 values:");
    for i in 0..8 {
        println!("  i={}: F32={:.6}, Q8={:.6}, diff={:.6}",
            i, out_f32[i], out_q8[i], out_f32[i] - out_q8[i]);
    }
    ...
    let conv_out: Vec<f32> = bx.iter().zip(kernel[2..].chunks_exact(3).next().unwrap().iter()).map(|(bxi, ki)| bxi * ki).take(2048).collect();
    ...
    let conv_out: Vec<f32> = bx.iter().zip(kernel_row2.iter()).map(|(bxi, ki)| bxi * ki).collect();
    ...
    let y: Vec<f32> = c.iter().zip(conv_out.iter()).map(|(ci, oi)| ci * oi).collect();
    println!("\ny first 8:");
    for i in 0..8 {
        println!("  y[{}] = {:.6}", i, y[i]);
    }
