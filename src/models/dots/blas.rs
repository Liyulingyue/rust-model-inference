...
//!
//! On macOS with the `accelerate` Cargo feature enabled, links against
//! Apple's Accelerate framework; on Linux x86_64 with the `openblas` Cargo
//! feature enabled, links against system OpenBLAS.
...
//! - `cargo build --features accelerate`        -- link Accelerate.framework
//!   on macOS.
//! - `cargo build --features openblas`          -- link against libopenblas.so
//!   on Linux x86_64.
//!
//! Call sites gate the BLAS path with the inline cfg predicate
//!   `#[cfg(any(all(feature = "accelerate", target_os = "macos"), all(feature = "openblas", target_os = "linux", target_arch = "x86_64")))]`
