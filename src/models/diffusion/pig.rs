...
impl PigConfig {
    pub fn from_source<S: crate::core::tensor::TensorSource + ?Sized>(
        source: &S,
    ) -> Result<Self, String> {
        let n_layer = source
...
impl PigModel {
    pub fn from_source(
        ...
        for i in 0..config.n_refiner_layers {
            context_refiner_layers.push(load_refiner_layer(
                source.as_ref(),
        ...
        for i in 0..config.n_refiner_layers {
            noise_refiner_layers.push(load_refiner_layer(
                source.as_ref(),
        ...
        let final_layer_adaln_weight =
            load_f16_as_f32(source.as_ref(), "final_layer.adaLN_modulation.1.weight")?;
        let final_layer_adaln_bias =
            load_f32(source.as_ref(), "final_layer.adaLN_modulation.1.bias")?;
        let final_layer_linear_weight =
            load_f16_as_f32(source.as_ref(), "final_layer.linear.weight")?;
...
fn load_refiner_layer(
    ...
    let adaln_weight =
        if let Ok(w) = load_f16_as_f32(source, &format!("{}.adaLN_modulation.0.weight", prefix)) {
            Some(w)
        } else {
            None
        };
...
impl PigVAE {
            ...
            for ky in 0..2 {
                for kx in 0..2 {
                    for c in 0..latent_channels {
                        ...
                        let dst_idx = (py * 2 + ky) * latent_size * latent_channels
                            + (px * 2 + kx) * latent_channels
                            + c;
                        ...
                        for ky in 0..kernel_size {
                            for kx in 0..kernel_size {
                                ...
                                if iy >= 0 && iy < h as i32 && ix >= 0 && ix < h as i32 {
                                    ...
                                    let weight_idx = oc * in_c * kernel_size * kernel_size
    ...
    fn group_norm(&self, input: &mut [f32], weight: &[f32], bias: &[f32], num_groups: usize) {
        ...
        for g in 0..num_groups {
            ...
            let var =
                group.iter().map(|x| (x - mean) * (x - mean)).sum::<f32>() / ch_per_group as f32;
            let inv_std = 1.0 / (var.sqrt() + 1e-5);
            for i in 0..ch_per_group {
                input[start + i] =
                    (input[start + i] - mean) * inv_std * weight[start + i] + bias[start + i];
            }
        }
    }
    ...
    fn upsample_nearest(
        ...
        for c in 0..channels {
            for y in 0..out_size {
                for x in 0..out_size {
                    output[c * out_size * out_size + y * out_size + x] =
                        input[c * in_size * in_size + (y / factor) * in_size + (x / factor)];
                }
            }
        }
    }
...
impl<'a> PigSession<'a> {
    pub fn new(model: &'a PigModel, resolution: usize) -> Result<Self, String> {
        if resolution % 8 != 0 || resolution == 0 {
            return Err(format!(
                "resolution {resolution} must be a positive multiple of 8"
            ));
        }
        Ok(Self {
            model,
    ...
    pub fn generate_image(
        ...
        for (step_idx, &sigma) in sigmas.iter().enumerate() {
            ...
            let denoised =
                self.denoise_step(&x_noisy, sigma, text_context, n_patches, patches_per_dim)?;

            let sigma_next = sigmas[step_idx + 1];
            let sigma_ratio = sigma_next / sigma;
            let updated: Vec<f32> = x_noisy
                .iter()
                .zip(denoised.iter())
                .map(|(&x, &d)| sigma_ratio * x + (1.0 - sigma_ratio) * d)
                .collect();
    ...
    fn cap_embedder(&self, text_context: &[f32], context_len: usize) -> Result<Vec<f32>, String> {
        ...
        for t in 0..n_tokens {
            let mean = text_context[t * cap_feat_dim..(t + 1) * cap_feat_dim]
                .iter()
                .sum::<f32>()
                / cap_feat_dim as f32;
            let var = text_context[t * cap_feat_dim..(t + 1) * cap_feat_dim]
                .iter()
                .map(|x| (x - mean) * (x - mean))
                .sum::<f32>()
                / cap_feat_dim as f32;
            ...
            for i in 0..cap_feat_dim {
                normed[t * cap_feat_dim + i] = (text_context[t * cap_feat_dim + i] - mean)
                    * inv_std
                    * self.model.cap_embedder_0_weight[i];
            }
        }
        ...
        for p in 0..n_patches {
            ...
            for i in 0..n_embed {
                ...
                for j in 0..patch_area {
                    sum += (patch_data[j] - mean)
                        * inv_std
                        * self.model.x_embedder_weight[j + i * patch_area];
                }
                out[p * n_embed + i] = sum;
            }
        }
    ...
    fn refiner_block(
        ...
        let normed: Vec<f32> = h
            .chunks_exact(n_embed)
            .flat_map(|chunk| {
                let mean = chunk.iter().sum::<f32>() / n_embed as f32;
                let var =
                    chunk.iter().map(|x| (x - mean) * (x - mean)).sum::<f32>() / n_embed as f32;
                let inv_std = 1.0 / (var.sqrt() + 1e-5);
                chunk
                    .iter()
                    .enumerate()
                    .map(|(i, &x)| (x - mean) * inv_std * layer.attention_norm1_weight[i])
                    .collect::<Vec<_>>()
            })
            .collect();
        ...
        for t in 0..n_tokens {
            ...
            let token_qkv =
                self.matmul_q8_single(&layer.qkv_weight, token_input, n_embed, n_embed * 3);
        ...
        for t in 0..n_tokens {
            ...
            for h_idx in 0..n_head_kv {
                ...
                k_all[t * n_head_kv * head_dim + h_idx * head_dim
                    ..t * n_head_kv * head_dim + (h_idx + 1) * head_dim]
                    .copy_from_slice(&qkv[k_src..k_src + head_dim]);
                v_all[t * n_head_kv * head_dim + h_idx * head_dim
                    ..t * n_head_kv * head_dim + (h_idx + 1) * head_dim]
        ...
        for t in 0..n_tokens {
            for h_idx in 0..n_head {
                ...
                let q_slice =
                    &mut q_all[t * n_head * head_dim + off..t * n_head * head_dim + off + head_dim];
                ...
                let k_slice = &mut k_all[t * n_head_kv * head_dim + kv_off
                    ..t * n_head_kv * head_dim + kv_off + head_dim];
                for i in 0..head_dim / 2 {
                    let cos = pe_slice[i * 2];
                    let sin = pe_slice[i * 2 + 1];
                    let cos2 = pe_slice[head_dim + i * 2];
    ...
    fn pig_layer_block(
        ...
        let adaln_mod =
            self.compute_adaln_modulation_q8(&layer.adaln_weight, &layer.adaln_bias, t_embed);
        ...
        for h_idx in 0..n_head {
            ...
            for i in 0..n_tokens {
                ...
                let q_slice = &mut q_all[i * n_head * head_dim + q_off_base
                    ..i * n_head * head_dim + q_off_base + head_dim];
                for d in 0..head_dim {
                    q_slice[d] = 0.0;
                }
                for j in 0..n_tokens {
        ...
        for t in 0..n_tokens {
            for h_idx in 0..n_head {
                attn_out[t * n_embed + h_idx * head_dim..t * n_embed + (h_idx + 1) * head_dim]
                    .copy_from_slice(
                        &q_all[t * n_head * head_dim + h_idx * head_dim
                            ..t * n_head * head_dim + (h_idx + 1) * head_dim],
                    );
            }
        }
        ...
        let attn_gated: Vec<f32> = modulated
            .iter()
            .zip(gate_tanh.iter())
            .map(|(m, g)| m * g)
            .collect();
        ...
        let ffn_normed: Vec<f32> = residual
            .chunks_exact(n_embed)
            .flat_map(|chunk| {
                let mean = chunk.iter().sum::<f32>() / n_embed as f32;
                let var =
                ...
                chunk
                    .iter()
                    .enumerate()
                    .map(|(i, &x)| (x - mean) * inv_std * layer.ffn_norm1_weight[i])
                    .collect::<Vec<_>>()
            })
            .collect();
        ...
        let ffn_gated: Vec<f32> = ffn_out
            .iter()
            .zip(scale_mlp_tanh.iter())
            .map(|(f, s)| f * s)
            .collect();
        let final_out: Vec<f32> = residual
            .iter()
            .zip(ffn_gated.iter())
            .map(|(&r, &f)| r + f)
            .collect();
    ...
    fn final_layer(&self, input: &[f32], t_embed: &[f32]) -> Result<Vec<f32>, String> {
        ...
        let var_all = input
            ...
            .sum::<f32>()
            / (n_patches * n_embed) as f32;
        let inv_std_all = 1.0 / (var_all.sqrt() + 1e-6);

        let normed: Vec<f32> = input
            .iter()
            .map(|&x| (x - mean_all) * inv_std_all)
            .collect();

        let modulated: Vec<f32> = normed
            .iter()
            .enumerate()
            .map(|(i, &x)| x * (1.0 + scale[i % n_embed]))
            .collect();
    ...
    fn compute_adaln_modulation_q8(
        &self,
        weight: &[u8],
        bias: &[f32],
        t_embed: &[f32],
    ...
    fn compute_adaln_modulation_from_f32(
        &self,
        weight: &[f32],
        bias: &[f32],
        t_embed: &[f32],
            ...
            for ky in 0..cfg.patch_size {
                for kx in 0..cfg.patch_size {
                    for c in 0..cfg.latent_channels {
                        let latent_idx = (start_y + ky) * latent_size * cfg.latent_channels
                            + (start_x + kx) * cfg.latent_channels
                            + c;
                        let patch_elem = ky * cfg.patch_size * cfg.latent_channels
                            + kx * cfg.latent_channels
                            + c;
