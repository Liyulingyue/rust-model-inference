...
//!
//! Extracted out of the previous free function `run_forward_logits_lfm2`
...
use super::forward::run_forward_logits_lfm2_with_batch;
use super::weights::{load_layers, Lfm2LayerWeights};
...
pub struct Lfm2Session<'a> {
    pub config: Lfm2Config,
    pub weights: Lfm2Weights<'a>,
...
pub struct Lfm2Weights<'a> {
    pub layers: Vec<Lfm2LayerWeights<'a>>,
...
impl<'a> Lfm2Session<'a> {
    ...
    pub fn from_source_with_max_rows(
        ...
        let config = Lfm2Config::from_source(source)
...
impl<'a> ChunkedPrefill for Lfm2Session<'a> {
    type Input = Vec<u32>;
    ...
    fn forward_chunk(
        ...
        if rows != input.len() {
            return Err(format!(
                "Lfm2Session::forward_chunk only handles whole-input chunks; \
                 rows = {rows} vs input.len() = {}",
                input.len()
            ));
        }
        if !project_logits {
            return Err("Lfm2Session::forward_chunk requires project_logits = true".into());
        }
        // At B = 1 the dispatch is bit-exact to the legacy per-token
        // walk. Delegate to the session's `forward_logits_chunked`
        // (which itself calls `run_forward_logits_lfm2_with_batch`).
...
mod tests {
    ...
    fn chunked_prefill_input_len_matches_token_count() {
        ...
        assert_eq!(<Lfm2Session<'_> as ChunkedPrefill>::input_len(&tokens), 7);
    }
    ...
    fn chunked_prefill_rejects_rows_above_one_for_now() {
        ...
        // do real `rows > 1` work.
        //
        // We can't construct a real `Lfm2Session` here without a
