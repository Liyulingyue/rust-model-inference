...
fn main() {
    let path = std::env::args()
        .nth(1)
        .expect("Usage: dump_emb <model.gguf> [token_id]");
    let token_id: u32 = std::env::args()
        .nth(2)
    ...
    for t in loader.tensors() {
        if t.name == "token_embd.weight" {
            println!(
                "Found token_embd.weight: dims={:?} type={:?}",
                t.dims, t.ggml_type
            );
            ...
            rust_model_inference::ops::embedding_lookup(
                weight,
                token_id,
                n_embd,
                t.ggml_type,
            ...
            println!(
                "First 16 embedding values for token {} (via embedding_lookup):",
                token_id
            );
            ...
            if let GGMLType::Q8_0 = t.ggml_type {
                ...
                for b in 0..4 {
                    ...
                    println!(
                        "  block {} off={} bytes={:#04x},{:#04x}",
                        b, off, scale_bytes[0], scale_bytes[1]
                    );
