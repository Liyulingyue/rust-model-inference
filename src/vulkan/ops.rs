...
pub fn run_qwen3_operator_check(context: &VulkanContext) -> Result<(), String> {
    ...
    for (index, value) in initial_k.iter_mut().enumerate() {
        *value = (index as f32 * 0.071).sin() * 0.5;
    }
    for (index, value) in initial_v.iter_mut().enumerate() {
        *value = (index as f32 * 0.053).cos() * 0.4;
    }
    ...
    let result = (|| -> Result<(), String> {
        ...
        let expected_projection = cpu_q8_matvec(&q_weight, &expected_q8, &expected_scales, N_EMBD);
    ...
    for head in 0..q_heads {
        ...
        for token in 0..sequence_length {
            ...
            scores[head * sequence_length + token] =
                crate::ops::dot_f32(query, &k[key_start..key_start + head_dim], head_dim) * scale;
        }
    }
    scores
}
...
fn cpu_attention_values(
    ...
    for head in 0..q_heads {
        ...
        for dimension in 0..head_dim {
            let mut sum = 0.0f32;
            for token in 0..sequence_length {
                sum += scores[head * sequence_length + token]
                    * values[token * kv_width + kv_head * head_dim + dimension];
            }
            output[head * head_dim + dimension] = sum;
        }
    }
    output
}
