...
use super::hsum_ps;
use super::has_avx2_fma;
use super::has_f16c;
use super::has_neon;
use super::norm::{rms_norm, rms_norm_inplace};
use super::quant::q8_0::quantize_q8_0_into;
use super::rope::{rope_mrope, rope_neox};
use super::vec_mad_f16_f32;

#[inline]
#[cfg(target_arch = "x86_64")]
pub unsafe fn _matmul_hsum_ps(v: std::arch::x86_64::__m256) -> f32 {
    use std::arch::x86_64::*;
...
pub fn matmul_q8_0_via_q8(
    ...
    #[cfg(target_arch = "x86_64")]
    {
        if has_avx2_fma() {
            unsafe {
                super::kernel::q8_0::avx2::matmul_q8_0_vs_q8_0_avx2(weight, q8_buf, scale_buf, output, n_in, 0, n_out);
            }
            return;
        }
    }
...
pub fn matmul_q8_0_quantized(
    ...
    #[cfg(target_arch = "x86_64")]
    {
        if has_avx2_fma() {
            unsafe {
                super::kernel::q8_0::avx2::matmul_q8_0_vs_q8_0_avx2(weight, input_q8, input_scales, output, n_in, 0, n_out);
            }
            return;
        }
    }
...
/// Pub(crate) for `kernel::f32::F32Kernel`. Do not call directly.
...
pub fn matmul_q8_0_quantized_parallel_rows(
    ...
    if nth <= 1 || n_out == 0 {
        super::kernel::q8_0::dispatch::matmul_q8_0_quantized_range(weight, input_q8, input_scales, output, n_in, 0, n_out);
        return;
    }
    ...
    super::kernel::q8_0::dispatch::matmul_q8_0_quantized_range(
        weight,
        input_q8,
        input_scales,
...
pub fn matmul_q8_0_quantized_dynamic(
    ...
    pool.compute_with_chunks(n_chunks, move |_ith, chunk_id| {
        ...
        super::kernel::q8_0::dispatch::matmul_q8_0_quantized_range(w, iq, sc, out_slice, n_in, row_start, row_end);
    });
}
...
fn parallel_range(
    ...
    if n <= min_rows {
        ...
        if use_avx2 {
            unsafe {
                super::kernel::q8_0::avx2::matmul_q8_0_vs_q8_0_avx2(
                    weight,
                    input_q8,
                    input_scales,
                    output,
...
pub fn matmul_q8_0(weight: &[u8], input: &[f32], output: &mut [f32], n_in: usize, n_out: usize) {
    #[cfg(target_arch = "x86_64")]
    {
        if has_avx2_fma() {
            unsafe {
                super::kernel::q8_0::avx2::matmul_q8_0_avx2_range(weight, input, output, n_in, 0, n_out);
            }
            return;
        }
    }
...
pub fn matmul_q8_0_parallel(
    ...
    output
        ...
        .for_each(|(i, out_slice)| {
            ...
            if use_avx2 {
                unsafe {
                    super::kernel::q8_0::avx2::matmul_q8_0_avx2_range(weight, input, out_slice, n_in, rs, re);
                }
                return;
            }
...
pub fn matmul_q8_0_batch(tasks: &mut [MatmulTask<'_>]) {
    ...
    work_items.par_iter().for_each(|&(ti, rs, re)| {
        ...
        if use_avx2 {
            unsafe {
                super::kernel::q8_0::avx2::matmul_q8_0_avx2_range(weight, input, out_slice, info.n_in, rs, re);
            }
            return;
        }
