...
use crate::ops::{
    dot_f16, dot_f16_f16_bytes, dot_f16_f32, f16_to_f32, f32_slice_to_f16, f32_to_f16,
...
fn neon_q8_matmul_matches_repacked_fused_block_accumulation() {
    ...
    unsafe {
        crate::ops::kernel::q8_0::neon::matmul_q8_0_vs_q8_0_neon(
            &weights,
            &input_q8,
            &input_scales,
            &mut output,
...
fn neon_f16_ops_match_scalar_with_tail() {
    ...
    assert_close(dot_f16_f32(&src, &bits, src.len()), expected_dot);
}
...
fn neon_q8_matmul_matches_scalar_for_partial_row_range() {
    ...
    unsafe {
        crate::ops::kernel::q8_0::neon::matmul_q8_0_vs_q8_0_neon(
            &weights, &q8, &scales, &mut neon, n_in, 1, 6,
        )
    };
    for i in 0..5 {
...
#[cfg(target_arch = "aarch64")]
#[test]
fn neon_q8_dotprod_nrc4_matches_ggml_repacked_4x8_accumulation() {
    if !std::arch::is_aarch64_feature_detected!("dotprod") {
        return;
    }
    ...
    const ROWS: usize = 4;
    let input_scales = [32768.0f32, 1.0, 32768.0, 0.5, 2.0];
    let mut input_q8 = vec![0u8; BLOCKS * 32];
    for block in 0..BLOCKS {
        for i in 0..32 {
            input_q8[block * 32 + i] = (((block * 29 + i * 17 + 11) % 127) as i16 - 63) as i8 as u8;
        }
    }

    let row_scales = [1.0f32, 0.5, 2.0, 0.25];
    let mut weights = Vec::with_capacity(ROWS * BLOCKS * 34);
    for (row, scale) in row_scales.into_iter().enumerate() {
        for block in 0..BLOCKS {
            weights.extend_from_slice(&f32_to_f16(scale).to_le_bytes());
            for i in 0..32 {
                weights
                    .push((((row * 41 + block * 23 + i * 13 + 7) % 127) as i16 - 63) as i8 as u8);
            }
        }
    }
    ...
    // Generated by llama.cpp 3173a56471c's ARM dotprod
    // ggml_gemv_q8_0_4x8_q8_0 kernel with the same packed blocks.
    assert_eq!(
        output.map(f32::to_bits),
        [0x4bc1_4df7, 0x4c97_7950, 0x4b1c_cab4, 0x4cab_4215]
    );
}
