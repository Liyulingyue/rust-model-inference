//! F16 × Q8_0 AVX2 (x86_64) matmul kernel.
//!
//! Companion to `bf16::avx2_q8` for the F16 weight path.  Weight bytes
//! are unpacked with `_mm256_cvtph_ps` (F16C); the Q8 input lands in
//! f32 via the same `_mm256_cvtepi8_epi32` + `_mm256_cvtepi32_ps`
//! sequence as the BF16 kernel.  Per-block Q8 scales are applied via a
//! blended f32x8 vector so the kernel handles cross-block AVX2 lanes
//! without splitting.
...
/// F16 × Q8_0 packed matmul.  Layout mirrors `scalar::forward_q8_rows`.
#[target_feature(enable = "avx2", enable = "fma", enable = "f16c")]
pub unsafe fn matmul_f16_vs_q8_avx2(
    weight: &[u8],
    input_q8: &[u8],
    input_scales: &[f32],
    output: &mut [f32],
    ...
    assert!(
        input_q8.len() >= n_in,
        "F16×Q8 AVX2: input_q8 len {} < n_in {n_in}",
        input_q8.len()
    );
    assert!(
        input_scales.len() >= n_in.div_ceil(32),
        "F16×Q8 AVX2: input_scales len {} < required {}",
        input_scales.len(),
        n_in.div_ceil(32)
    );
...
#[cfg(test)]
mod tests {
    use super::matmul_f16_vs_q8_avx2;
