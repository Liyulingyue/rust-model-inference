//! F32×F32 AVX2 (x86_64) matmul kernel.
//!
//! Strategy mirrors `bf16/avx2.rs` but with the unpack step removed — F32
//! weights are loaded directly as `__m256`.  We process 32 F32 weight × 32
//! F32 input per iteration in 4 accumulators (8-wide each), then hsum to a
//! single f32 per row.
//!
//! **Precision contract**: bit-exact with `f32::scalar::forward_f32_rows`.

#![cfg(target_arch = "x86_64")]

#[target_feature(enable = "avx2", enable = "fma")]
pub unsafe fn matmul_f32_vs_f32_avx2(
    weight: &[f32],
    input: &[f32],
    output: &mut [f32],
    n_in: usize,
    row_start: usize,
    row_end: usize,
) {
    use std::arch::x86_64::*;

    debug_assert_eq!(n_in % 8, 0);
    let w_ptr = weight.as_ptr();
    let i_ptr = input.as_ptr();
    let out_ptr = output.as_mut_ptr();

    let mut out_local = 0usize;
    for row in row_start..row_end {
        let row_off = row * n_in;
        let mut acc0 = _mm256_setzero_ps();
        let mut acc1 = _mm256_setzero_ps();
        let mut acc2 = _mm256_setzero_ps();
        let mut acc3 = _mm256_setzero_ps();

        let mut i = 0;
        while i + 32 <= n_in {
            let w0 = _mm256_loadu_ps(w_ptr.add(row_off + i));
            let w1 = _mm256_loadu_ps(w_ptr.add(row_off + i + 8));
            let w2 = _mm256_loadu_ps(w_ptr.add(row_off + i + 16));
            let w3 = _mm256_loadu_ps(w_ptr.add(row_off + i + 24));

            let x0 = _mm256_loadu_ps(i_ptr.add(i));
            let x1 = _mm256_loadu_ps(i_ptr.add(i + 8));
            let x2 = _mm256_loadu_ps(i_ptr.add(i + 16));
            let x3 = _mm256_loadu_ps(i_ptr.add(i + 24));

            acc0 = _mm256_fmadd_ps(w0, x0, acc0);
            acc1 = _mm256_fmadd_ps(w1, x1, acc1);
            acc2 = _mm256_fmadd_ps(w2, x2, acc2);
            acc3 = _mm256_fmadd_ps(w3, x3, acc3);

            i += 32;
        }
        while i + 8 <= n_in {
            let w_f = _mm256_loadu_ps(w_ptr.add(row_off + i));
            let x_f = _mm256_loadu_ps(i_ptr.add(i));
            acc0 = _mm256_fmadd_ps(w_f, x_f, acc0);
            i += 8;
        }
        while i < n_in {
            let w_val = *w_ptr.add(row_off + i);
            let x_val = *i_ptr.add(i);
            acc0 = _mm256_fmadd_ps(_mm256_set1_ps(w_val), _mm256_set1_ps(x_val), acc0);
            i += 1;
        }

        let total = hsum256(acc0) + hsum256(acc1) + hsum256(acc2) + hsum256(acc3);
        *out_ptr.add(out_local) = total;
        out_local += 1;
    }
}

#[inline(always)]
unsafe fn hsum256(v: std::arch::x86_64::__m256) -> f32 {
    use std::arch::x86_64::*;
    let hi = _mm256_extractf128_ps(v, 1);
    let lo = _mm256_castps256_ps128(v);
    let s = _mm_add_ps(hi, lo);
    let sh = _mm_movehdup_ps(s);
    let sums = _mm_add_ps(s, sh);
    let shuf = _mm_movehl_ps(sums, sums);
    let final_sum = _mm_add_ss(sums, shuf);
    _mm_cvtss_f32(final_sum)
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::ops::kernel::f32::scalar::forward_f32_rows;

    fn assert_avx2_eq_scalar(label: &str, weight: &[f32], input: &[f32], n_in: usize, n_out: usize) {
        let mut avx2_out = vec![0.0f32; n_out];
        let mut scalar_out = vec![0.0f32; n_out];
        unsafe {
            matmul_f32_vs_f32_avx2(weight, input, &mut avx2_out, n_in, 0, n_out);
        }
        forward_f32_rows(weight, input, &mut scalar_out, n_in, n_out, 0, 1);
        // FMA reduction order (4×8-wide accumulators) differs from scalar
        // (1 wide accumulator); allow a small float accumulation tolerance,
        // matching the BF16 kernel's `1e-2` style.
        for (i, (a, s)) in avx2_out.iter().zip(scalar_out.iter()).enumerate() {
            let denom = s.abs().max(1e-3);
            let rel = (a - s).abs() / denom;
            assert!(
                rel < 1e-3 || (a - s).abs() < 1e-4,
                "{} row {}: avx2={} scalar={} diff={}",
                label,
                i,
                a,
                s,
                (a - s).abs(),
            );
        }
    }

    #[test]
    fn avx2_matches_scalar_small() {
        let n_in = 8;
        let n_out = 4;
        let weight: Vec<f32> = (0..n_in * n_out).map(|i| (i as f32 * 0.1) - 0.3).collect();
        let input: Vec<f32> = (0..n_in).map(|i| (i as f32 - 4.0) * 0.05).collect();
        assert_avx2_eq_scalar("small 4x8", &weight, &input, n_in, n_out);
    }

    #[test]
    fn avx2_matches_scalar_large() {
        let n_in = 256;
        let n_out = 16;
        let weight: Vec<f32> = (0..n_in * n_out)
            .map(|i| (((i * 13 + 7) % 47) as f32 - 23.0) * 0.01)
            .collect();
        let input: Vec<f32> = (0..n_in).map(|i| ((i % 23) as f32 - 11.0) * 0.02).collect();
        assert_avx2_eq_scalar("large 16x256", &weight, &input, n_in, n_out);
    }

    #[test]
    fn avx2_matches_scalar_tail() {
        let n_in = 40;
        let n_out = 3;
        let weight: Vec<f32> = (0..n_in * n_out)
            .map(|i| ((i * 17) as f32 * 0.013).sin())
            .collect();
        let input: Vec<f32> = (0..n_in).map(|i| (i as f32 - 20.0) * 0.03).collect();
        assert_avx2_eq_scalar("tail 3x40", &weight, &input, n_in, n_out);
    }
}
