...
unsafe fn rope_neox_inplace_avx2(
    ...
    for h in 0..n_heads {
        ...
        while i + 8 <= half {
            ...
            let prod_hi = _mm256_mul_ps(x_hi, sin_v);
            let new_lo = _mm256_sub_ps(prod_lo, prod_hi);
...
unsafe fn rope_neox_inplace_neon(
    ...
    for h in 0..n_heads {
        ...
        while i + 4 <= half {
            ...
            let new_lo = vsubq_f32(vmulq_f32(x_lo, cos_v), vmulq_f32(x_hi, sin_v));
            let new_hi = vaddq_f32(vmulq_f32(x_lo, sin_v), vmulq_f32(x_hi, cos_v));
