...
pub fn model_config_from_source<S: TensorSource + ?Sized>(
    ...
    if !matches!(prefix, "qwen2" | "qwen3" | "qwen3vl" | "qwen35" | "qwen3tts" | "llama" | "granite" | "hunyuan-dense" | "pig" | "lfm2" | "lfm2moe") {
    ...
    Ok(ModelConfig {
        ...
        n_head,
        n_head_kv: if arch == "lfm2" || arch == "lfm2moe" {
            // LFM2-family stores head_count_kv as a per-layer array; the
            // attention layers (kv=8) are the ones that matter for
...
///
/// Phase 4c: extracted from `models::qwen3::base::Qwen3Config::from_source` so that
...
/// called by `models::qwen3::base::Qwen3Config::from_source` after
