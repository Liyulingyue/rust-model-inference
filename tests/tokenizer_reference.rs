...
fn fixed_qwen_literal_ids_match_pinned_llama_cpp() {
    ...
    let qwen2 =
        BPETokenizer::from_gguf_metadata(|key| qwen2_loader.metadata(key).cloned()).unwrap();
    ...
    let qwen35 =
        BPETokenizer::from_gguf_metadata(|key| qwen35_loader.metadata(key).cloned()).unwrap();
...
fn qwen_chat_prompt_matches_reference_ids() {
    ...
    assert_eq!(
        ...
        .unwrap(),
        vec![151644, 872, 198, 9707, 151645, 198, 151644, 77091, 198,],
    );
}
...
fn qwen_system_user_assistant_prompt_matches_reference_ids() {
    ...
    assert_eq!(
        ...
        .unwrap(),
        vec![
            151644, 8948, 198, 2610, 525, 63594, 13, 151645, 198, 151644, 872, 198, 9707, 151645,
            198, 151644, 77091, 198, 90236, 3556, 13, 151645, 198, 151644, 77091, 198,
        ],
    );
}
