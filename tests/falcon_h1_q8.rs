...
fn model_loader_3b() -> Option<GGUFLoader> {
    let path = std::env::var_os("RMI_FALCON_H1_3B_Q8_MODEL")?;
    Some(GGUFLoader::from_file(path).unwrap())
}

fn model_source_3b() -> Option<Arc<dyn TensorSource>> {
    model_loader_3b().map(|inner| Arc::new(inner) as Arc<dyn TensorSource>)
}

fn falcon_tokenizer_3b() -> Option<BPETokenizer> {
    let loader = model_loader_3b()?;
    Some(
        BPETokenizer::from_gguf_metadata(|k| loader.metadata(k).cloned())
            .expect("falcon-h1 3B tokenizer must load"),
    )
}
...
fn q8_contract_loads_3b() {
    ...
    assert_eq!(config.ssm_inner_size, 4096);
    assert_eq!(config.ssm_time_step_rank, 32);
    assert_eq!(config.ssm_n_head(), 32);
    assert_eq!(config.ssm_headdim(), 128);
    assert_eq!(config.ssm_conv_cols(), 4608);
    assert_eq!(config.ssm_in_proj_dim(), 8736);
    let model = FalconH1Model::from_source(source, 4).unwrap();
    assert_eq!(model.layers.len(), 32);
    let layer = &model.layers[0];
    assert_eq!(layer.wq.n_in, 2560);
    assert_eq!(layer.wq.n_out, 1280);
    assert_eq!(layer.wk.n_out, 256);
    assert_eq!(layer.wv.n_out, 256);
    assert_eq!(layer.wo.n_in, 1280);
    assert_eq!(layer.ssm_in.n_out, 8736);
    assert_eq!(layer.ssm_out.n_in, 4096);
}
...
fn q8_chunked_and_incremental_logits_match_3b() {
    ...
    assert!(mismatch.is_none(), "3B first logit mismatch: {mismatch:?}");
}
...
fn q8_rejects_missing_attention_projection_3b() {
    ...
    assert!(error.contains("layer 16: attn_output"), "{error}");
}
...
fn q8_hello_greedy_3b() {
    ...
    // Pinned once both engines agree on the 3B Q8 matmul path; left
    // unconstrained until we lock in a parity dump.
    let _ = hash;
