...
| NVIDIA Nemotron-3 Nano 4B | `nemotron_h` | 文本生成 | 无 | **未**任何未接入 | `Unsupported` | Hybrid Mamba-Transformer 架构（40 个注意力头 + `ssm_*` Mamba2 层），共 42 层、`embedding_length=3136`。架构未注册到 `src/main.rs` 路由表；tokenizer `ggml.pre=pixtral` 未加入 `src/core/tokenizer.rs:367` 白名单。当前 `--prompt "Hello"` 立即失败，错误：`Unsupported tokenizer.ggml.pre "pixtral"`。要接入需要：(1) 新建 `src/models/nemotron_h/trunk/`（hybrid 注意力 + Mamba2 SSM 分发）；(2) 在 `src/ops/ssm.rs` 基础上扩展 Mamba2 dispatcher；(3) 加 `pixtral` / GPT-2 BPE tokenizer 分支；(4) 写 parity 测试对照 pinned llama.cpp commit。整体工作量 ~1500 行新代码 + 一份 llama.cpp Oracle pin。 |
