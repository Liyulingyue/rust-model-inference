...
| NVIDIA Nemotron-3 Nano 4B | `nemotron_h` | 文本生成 | 无 | tokenizer `pixtral` 接受（映射到 GPT-2 BPE）；架构已注册到 `src/main.rs` 与 `src/core/loader.rs`；新增 `src/models/nemotron_h/trunk/` 加载所有 42 层权重并跑通 attention + FFN + **canonical Mamba2 SSM**。 | `Experimental`（first-coherent-output milestone 已达成） | **Mamba2 scan 现在与 llama.cpp `ggml_compute_forward_ssm_scan_f32` 结构对齐**：`ssm_in` 形状 `[n_embd, d_in_proj=17504]` 正确，`ssm_conv1d` 是带 4-tap 历史 buffer 的真实 causal depthwise 卷积，scan 状态 shape 是 `(n_head=96, headdim=80, d_state=128)`（per-layer 983040 floats × 42 layers ≈ 165 MB），每 head 用 `decay = exp(dt · A_log)`（A 直接用 raw 值），B/C 是 per-group d_state 向量，输出 `y[h,k] = sum_n(state · C) + D·x`。已加 K/V cache 写入 `scratch.k/v`、conv1d history buffer、scan state buffer。Pinned oracle：commit `96013c511` (build 10120)，详见 [`docs/REFERENCE_IMPLEMENTATIONS.md`](../REFERENCE_IMPLEMENTATIONS.md) 和 [`docs/parity_fixtures/nemotron_h_4b/README.md`](../parity_fixtures/nemotron_h_4b/README.md)。`tests/nemotron_h_parity.rs` 三个 `#[ignore]`'d 测试可用 `--include-ignored` 跑出当前 diff。**剩余问题**：残差 L2 仍增长 ~500× (initial ~6 → layer 41 ~3000)，模型输出 coherent English 但不匹配 oracle（例：`Hello` → `,} believing Doudur asymptomatic DouglasBe hostallyuriszb...`，oracle 是 `Hello! How can I assist you today?`）。怀疑是 Q8 量化的 FFN/attention 残差累积。 |
