...
| NVIDIA Nemotron-3 Nano 4B | `nemotron_h` | 文本生成 | 无 | tokenizer `pixtral` 接受（映射到 GPT-2 BPE）；架构已注册到 `src/main.rs` 与 `src/core/loader.rs`；新增 `src/models/nemotron_h/trunk/` 加载所有 42 层权重并跑通 attention + FFN + **SSM 三条分支**。 | `Experimental` | **Mamba2 SSM 分支是结构上完整但功能上降级**：跑 in_proj → causal conv1d → SiLU → D-skip → group RMSNorm → out_proj，但**没有选择性扫描**（B/C/dt 投影矩阵未实现，所以不能 state-recurrent over A·dt）。`--prompt "Hello"` 输出 `_On_On_On_On_On_On_On_On`（重复 token，因为 D-skip 没有 scan 是 identity-like）。需要：(1) 实现 scan：`for s in 0..L: state = exp(A·dt) ⊙ state + dt ⊙ B ⊗ x; y = C·state + D·x`；(2) 写 B/C/dt 投影矩阵（目前 GGUF 没有 `ssm_b_proj` / `ssm_c_proj` / `ssm_dt_proj_back` 张量，需确认 llama.cpp 是怎么 broadcast time_step_rank → inner_size 的）。工作：~200 行新 dispatcher + 1 份 llama.cpp Oracle pin。 |
