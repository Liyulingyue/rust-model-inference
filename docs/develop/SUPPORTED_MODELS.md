...
| NVIDIA Nemotron-3 Nano 4B | `nemotron_h` | 文本生成 | 无 | tokenizer `pixtral` 接受（映射到 GPT-2 BPE）；架构已注册到 `src/main.rs` 与 `src/core/loader.rs`；新增 `src/models/nemotron_h/trunk/` 加载所有 42 层权重并跑通 attention + FFN 两条分支。 | `Experimental` | **Mamba2 SSM 分支尚未实现**（`ssm_in` / `ssm_conv1d` / `ssm_dt` / `ssm_a` / `ssm_d` / `ssm_norm` / `ssm_out` 权重已加载但 forward 为 no-op），21/42 层是纯 SSM 层因此输出为 garbled 文本（`--prompt "Hello"` 输出 `Kass univariatewealth graduated ill`）。需要：(1) 写 Mamba2 选择性扫描 dispatcher（`ssm_in` → conv1d → `softplus(dt)·A` → scan → group_norm → `ssm_out`）；(2) 与 llama.cpp `nemotron_h.cpp` 的 pinned commit 做 token-level parity test。每层 forward 在 current scratch layout 下 ~2-3 GB，工作量 ~600 行新 dispatcher + 1 份 Oracle pin。 |
