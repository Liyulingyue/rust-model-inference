...
| NVIDIA Nemotron-3 Nano 4B | `nemotron_h` | 文本生成 | 无 | tokenizer `pixtral` 接受（映射到 GPT-2 BPE）；架构已注册到 `src/main.rs` 与 `src/core/loader.rs`；新增 `src/models/nemotron_h/trunk/` 加载所有 42 层权重并跑通 attention + FFN + **SSM 三条分支**。 | `Experimental` | **4B Nano 的 GGUF 张量布局跟 llama.cpp `nemotron-h.cpp` 不完全一致**：llama.cpp 期望 `ssm_in = [n_embd, d_in_proj=17504]`（= 2*d_inner + 2*n_group*d_state + dt_rank），但本仓库的 `ssm_in` 是 `[7680, 3136] = [d_inner, n_embd]`——只有 x 投影，没有 z/B/C/dt 拼接。当前 forward 跑 in_proj → causal conv1d → SiLU → D-skip → group RMSNorm → out_proj，但**没有选择性扫描**（`ssm_conv1d` 的 [7680+1024..7680+2048] 通道虽然 load 进来但没用上）。`--prompt "Hello"` 输出 `_On_On_On_On_On_On_On_On`（重复 token，因为 D-skip 没有 scan 是 identity-like）。要跟 llama.cpp 完全对齐：(1) 写一个跟本仓库 GGUF 张量布局匹配的 Mamba2 dispatcher：把 `ssm_conv1d` 的 [d_inner + 2*1024] 拆成 [x_conv, B, C]，把 `ssm_dt` / `ssm_a` / `ssm_d` 从 `time_step_rank=96` broadcast 到 `n_group=8 × d_state=128`；(2) 写真正的 scan：`state = exp(A·dt) ⊙ state + dt ⊙ B ⊗ x; y = C·state + D·x`；(3) 与 llama.cpp `nemotron-h.cpp` 的 pinned commit 做 parity test。工作量 ~300 行新 dispatcher + 1 份 Oracle pin。 |
