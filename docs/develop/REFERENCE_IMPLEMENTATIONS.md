...
| Falcon-H1 1.5B / 3B Instruct | `171e8846b4af9766c354064cb776cb34a50f053f`（本地 checkout 当前 HEAD） | 本地 `references/llama.cpp/build-rmi-falconh1/bin/{llama-cli,llama-tokenize,llama-completion}`（scalar-no-OpenMP 构建，遵循 [`tools/oracle/shared/build_llama_oracle.sh`](shared/build_llama_oracle.sh) 标志，额外 `-DLLAMA_BUILD_SERVER=ON -DLLAMA_BUILD_TOOLS=ON` 以触发 llama-cli 构建）；对应 [`tests/falcon_h1_q8.rs`](../../tests/falcon_h1_q8.rs)。**状态**：Q8_0 仅在 `RMI_FALCON_H1_Q8_MODEL`（1.5B）或 `RMI_FALCON_H1_3B_Q8_MODEL`（3B）指向本地 `models/Falcon-H1-*-Instruct-GGUF/Falcon-H1-*-Instruct-Q8_0.gguf` 时运行；1.5B/3B **架构同型**（仅 n_embd/n_layer/n_head/n_ff/ssm_inner_size/ssm_dt_rank 不同）—— 同 trunk、零代码改动；分词器逐位对齐 oracle；张量级 Q8 matmul 与 `hsum_float_8` 归约顺序仍存在 ulp 级差异（受全局 `hsum_ps` 复用约束，后续可提本地 matmul） |
| Qwen3-Reranker 0.6B Instruct | `171e8846b4af9766c354064cb776cb34a50f053f`（同 qwen3 oracle） | 复用 `src/models/qwen3/trunk` + `cls.output.weight` 头；ChatML prompt（system + `<Instruct>/<Query>/<Document>`）由 [`src/bin/qwen3_rerank.rs`](../../src/bin/qwen3_rerank.rs) 内置；对应 [`tests/qwen3_rerank.rs`](../../tests/qwen3_rerank.rs)。**状态**：Q8_0 仅在 `RMI_QWEN3_RERANK_Q8_MODEL` 指向 `models/Qwen3-Reranker-0.6B-QGUF/qwen3-reranker-0.6b-q8_0.gguf`（来自 `ggml-org/Qwen3-Reranker-0.6B-Q8_0-GGUF`）时运行；做的是（query, doc）对的二分类头打分（yes/no），不调 lm_head，token 序列与 lm_head logits 与 oracle 不可直接对照；位置（yes logit）相对语义正确性用 `Paris vs Photosynthesis` 距离作 smoke test |

**HTTP API**：自动检测 `pooling_type=4` 切到 `/v1/rerank` 路由（[src/app/server/rerank.rs](../../src/app/server/rerank.rs)）。Request/response schema 对标 Cohere v1：`POST /v1/rerank {"query","documents","top_n","max_tokens_per_doc","return_documents"}`，响应 `{"id","model","results":[{"index","relevance_score"}]}`。Per-request 新建 `Qwen3Session`（每 doc 串行 prefill + head matmul），对前 `[tests/qwen3_rerank_http.rs](../../tests/qwen3_rerank_http.rs) 集成测试通过（起 axum server → curl POST → 验证 schema + Paris vs Photosynthesis 排序）。文档**未做批量 prefill**——多 doc 同 batch 共享 query 段 attention 的优化属于后续（低优先，预计 ~30+ docs 才值得做） | 
## 本地 checkout 约定
