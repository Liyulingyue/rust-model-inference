...
- [x] **Vulkan GPU matmul 后端可用（2026-08）** — 权重常驻 + 持久 IO 缓冲 + 单 dispatch 全行覆盖 + 看门狗。正确性（GPU vs CPU rel ≤ 3e-7）与稳定性（不再挂死）已验证，详见 `docs/develop/VULKAN.md`。剩余为 dispatch 开销优化（合并 dispatch / 批量提交 / dp4a），见 VULKAN.md 性能分析。
